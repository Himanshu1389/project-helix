...
def load_data():
    ...
    for col in ['salary', 'experience', 'budget']:
...
# Settings Page
    ...
    st.write("Configure application-wide settings for compensation planning.")

    # Load current settings from session state
    current_target_comp_budget = st.session_state.target_comp_budget
    current_currency_symbol = st.session_state.currency_symbol
    current_organization_name = st.session_state.organization_name

    st.subheader("Compensation Budget Settings")
    current_total_salary = employee_df['salary'].sum()
    st.metric("Current Total Compensation Budget", f"₹{current_total_salary:,.0f}")

    new_target_comp_budget = st.number_input(
        "Target Compensation Budget (Absolute Value)",
        min_value=0.0,
        value=float(current_target_comp_budget),
        step=100000.0,
        format="%.0f"
    )
    ...
    # Save settings if any value has changed
    if (new_target_comp_budget != current_target_comp_budget or
        new_currency_symbol != current_currency_symbol or
        new_organization_name != current_organization_name):

        st.session_state.target_comp_budget = new_target_comp_budget
        st.session_state.currency_symbol = new_currency_symbol
        st.session_state.organization_name = new_organization_name

        # Save to CSV
        settings_data = {
            'company_name': [new_organization_name],
        ...
        st.success("Settings updated and saved successfully! Please refresh the page to see changes applied everywhere.")
        st.experimental_rerun()

# Footer
st.markdown("---")
st.markdown("Helix™ is an AI-native platform for strategic compensation management in India.") 